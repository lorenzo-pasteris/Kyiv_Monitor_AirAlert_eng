...
class StatsDbDisabledTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        monitor.CATEGORY_STATS_DB_PATH = str(Path(self.temp_dir.name) / "unused.sqlite3")
    ...
    def test_persist_operational_state_short_circuits(self):
        self.assertFalse(monitor.persist_operational_state("key", "value"))
        self.assertFalse(Path(monitor.CATEGORY_STATS_DB_PATH).exists())

    def test_load_operational_state_short_circuits(self):
        self.assertIsNone(monitor.load_operational_state("key"))

    def test_persist_trigger_observation_short_circuits(self):
        self.assertFalse(
            monitor.persist_trigger_observation(True, 1, datetime.now(timezone.utc))
        )

    def test_alert_feed_cursor_helpers_short_circuit(self):
        self.assertEqual(monitor.get_alert_feed_cursor("kyiv_alerts"), 0)
        self.assertFalse(monitor.set_alert_feed_cursor("kyiv_alerts", 42))
    ...
    def test_claim_alert_feed_delivery_fails_open_when_db_unavailable(self):
        ...
        self.assertTrue(monitor.claim_alert_feed_delivery("kyiv_alerts", 1, "text"))
        self.assertTrue(monitor.claim_alert_feed_delivery("kyiv_alerts", 1, "text"))

    def test_release_alert_feed_delivery_is_a_safe_noop(self):
        self.assertIsNone(monitor.release_alert_feed_delivery("kyiv_alerts", 1, "text"))

    def test_is_stale_alert_feed_message_defaults_to_not_stale(self):
        self.assertFalse(monitor.is_stale_alert_feed_message("kyiv_alerts", 999))
    ...
    def test_get_source_cursor_short_circuits(self):
        self.assertIsNone(monitor.get_source_cursor("channel"))

    def test_persist_history_batch_short_circuits(self):
        self.assertFalse(monitor.persist_history_batch("channel", [], 5))
    ...
    def test_mark_normal_messages_processed_is_a_safe_noop(self):
        ...
        self.assertIsNone(monitor.mark_normal_messages_processed([("channel", 1)]))

    def test_discard_pending_normal_messages_is_a_safe_noop(self):
        self.assertIsNone(monitor.discard_pending_normal_messages("unit-test"))

    def test_persist_category_stats_is_a_safe_noop(self):
        self.assertIsNone(monitor.persist_category_stats("2026-01-01T00:00:00+00:00", [], {}))
    ...
    def test_initialize_stats_db_still_works_when_flag_is_false(self):
        ...
        self.assertTrue(monitor.initialize_stats_db())
        self.assertTrue(Path(monitor.CATEGORY_STATS_DB_PATH).exists())
...
class PersistenceEdgeCaseTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_operational_state_survives_simulated_restart(self):
        self.assertTrue(monitor.persist_operational_state("some_key", "some_value"))
        ...
        with sqlite3.connect(monitor.CATEGORY_STATS_DB_PATH) as fresh_connection:
        ...
        self.assertEqual(monitor.load_operational_state("some_key"), "some_value")

    def test_alert_feed_cursor_survives_simulated_restart(self):
        self.assertTrue(monitor.set_alert_feed_cursor("kyiv_alerts", 555))

        with sqlite3.connect(monitor.CATEGORY_STATS_DB_PATH) as fresh_connection:
            row = fresh_connection.execute(
                "SELECT value FROM operational_state WHERE key = ?",
                (monitor.alert_feed_cursor_key("kyiv_alerts"),),
            ).fetchone()
        self.assertEqual(row[0], "555")
        self.assertEqual(monitor.get_alert_feed_cursor("kyiv_alerts"), 555)

    # -- malformed / missing operational_state values ------------------------------

    def test_get_alert_feed_cursor_handles_missing_value(self):
        self.assertEqual(monitor.get_alert_feed_cursor("never-seen-channel"), 0)

    def test_get_alert_feed_cursor_handles_non_integer_stored_value(self):
        monitor.persist_operational_state(
            monitor.alert_feed_cursor_key("kyiv_alerts"), "not-an-int"
        )
    ...
    def test_is_stale_alert_feed_message_with_malformed_cursor(self):
        monitor.persist_operational_state(
            monitor.alert_feed_cursor_key("kyiv_alerts"), "garbage"
        )
        # A malformed cursor is treated as 0 (missing), so nothing is considered stale.
        self.assertFalse(monitor.is_stale_alert_feed_message("kyiv_alerts", 1))
    ...
    def test_claim_alert_feed_delivery_only_one_concurrent_caller_wins(self):
        ...
        results = [
            monitor.claim_alert_feed_delivery("kyiv_alerts", 42, text)
            for _ in range(5)
        ]
        # Exactly one of five "concurrent" claims for the same message/version wins.
    ...
    def test_release_allows_retry_after_failed_claim(self):
        ...
        self.assertTrue(monitor.claim_alert_feed_delivery("kyiv_alerts", 7, text))
        self.assertFalse(monitor.claim_alert_feed_delivery("kyiv_alerts", 7, text))

        monitor.release_alert_feed_delivery("kyiv_alerts", 7, text)
    ...
    def test_release_of_unclaimed_delivery_is_a_harmless_noop(self):
        ...
        monitor.release_alert_feed_delivery("kyiv_alerts", 999, "never claimed")
        self.assertTrue(monitor.claim_alert_feed_delivery("kyiv_alerts", 999, "never claimed"))

    def test_is_stale_alert_feed_message_reflects_cursor(self):
        monitor.set_alert_feed_cursor("kyiv_alerts", 100)
        self.assertTrue(monitor.is_stale_alert_feed_message("kyiv_alerts", 99))
        self.assertTrue(monitor.is_stale_alert_feed_message("kyiv_alerts", 100))
        self.assertFalse(monitor.is_stale_alert_feed_message("kyiv_alerts", 101))
    ...
    def test_mark_normal_messages_processed_only_marks_exact_keys(self):
        ...
        monitor.persist_normal_message("chan_a", 1, now, "first message body text")
        monitor.persist_normal_message("chan_a", 2, now, "second message body text")
        monitor.persist_normal_message("chan_b", 1, now, "other channel message text")

        monitor.mark_normal_messages_processed([("chan_a", 1)])
    ...
    def test_mark_normal_messages_processed_with_empty_list_is_noop(self):
        now = datetime.now(timezone.utc)
        monitor.persist_normal_message("chan_a", 1, now, "message body text here")
        monitor.mark_normal_messages_processed([])
        self.assertEqual(len(monitor.load_pending_normal_messages()), 1)

    def test_mark_normal_messages_processed_deletes_rows_past_retention(self):
        old_processed_at = monitor.utc_iso(
            datetime.now(timezone.utc)
            - timedelta(days=monitor.NORMAL_MESSAGE_RETENTION_DAYS + 1)
        )
        ...
        monitor.mark_normal_messages_processed([("chan_pending", 3)])
    ...
    def test_discard_pending_normal_messages_leaves_processed_rows_untouched(self):
        now = datetime.now(timezone.utc)
        monitor.persist_normal_message("chan_a", 1, now, "message that will be discarded")
        monitor.persist_normal_message("chan_a", 2, now, "message that gets processed")
        monitor.mark_normal_messages_processed([("chan_a", 2)])

        monitor.discard_pending_normal_messages("unit-test")
    ...
    def test_persist_category_stats_writes_rows_for_each_channel(self):
        ...
        monitor.persist_category_stats(run_at, snapshots, category_results)
    ...
    def test_telethon_session_lock_blocks_second_non_blocking_acquire(self):
        ...
        holder = monitor.acquire_telethon_session_lock(path)
        try:
        ...
        second_holder = monitor.acquire_telethon_session_lock(path, blocking=False)
        second_holder.close()

    def test_telethon_session_lock_creates_parent_directory(self):
        path = str(Path(self.temp_dir.name) / "nested" / "dir" / "telethon.lock")
        holder = monitor.acquire_telethon_session_lock(path, blocking=False)
        try:
