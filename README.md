...
`httpcore`, and is what Railway actually installs from (see `railway.json`), so every
deploy gets the exact versions that were tested. Regenerate it after changing
`requirements.txt`:
